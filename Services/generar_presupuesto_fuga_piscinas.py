...
def generar_presupuesto_fuga_piscinas(cliente: "Cliente", presupuesto: "Presupuesto_Fuga_Piscinas") -> True:
    ...
    document.paragraphs[8].add_run(
        f" {cliente.localidad}"
        + " " * 10
        + f"CP: {cliente.cp}"
        + " " * 10
        + f"PROVINCIA: {cliente.provincia}"
    ).font.color.rgb = font_color
    document.paragraphs[8].runs[1].bold = True

    document.paragraphs[18].add_run(
        f" {presupuesto.ancho}x{presupuesto.largo}m."
    ).font.color.rgb = font_color
    ...
    document.paragraphs[49].add_run(
        f"{floor((presupuesto.total - presupuesto.precio) * 100)/100.0}0"
    ).font.color.rgb = font_color
